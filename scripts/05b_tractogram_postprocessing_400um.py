#!/usr/bin/env python
"""Post-process raw tractography streamlines (400um).

Loads raw streamlines (.npy) from 05_tractography_400um.py, rescales them
from dMRI voxel space to HiP-CT level-4 voxel space, converts to RASMM
via the reference NIfTI affine, compresses, and saves as .trk. Optionally
filters streamlines by z-range and minimum physical length.

This is the 400um variant of 05b_tractogram_postprocessing.py. The
pipeline logic is identical; only the configuration section differs.
"""
...
if __name__ == "__main__":
    ...
    COMPRESS_TOL = 0.11        # mm  (0.1 in 800um version)
    MIN_LENGTH_MM = 20.0       # filter streamlines shorter than this
    Z_RANGE = (200, 450)       # filter by average z in voxel space  ((200, 500) in 800um version)
    ...
    # ---- Find raw streamlines file (400um-specific filename prefix) ----
    raw_pattern = f"{DMRI_RESOLUTION}um_raw_*_streamlines_{START_IDX}_{END_IDX}_{GFA_THRESHOLD}_*.npy"
    ...
    # ---- Load SH coefficients shape for scaling (400um-specific zarr name) ----
    ...
    out_stem = raw_fname.stem.replace(f"{DMRI_RESOLUTION}um_raw_", f"{DMRI_RESOLUTION}um_")
